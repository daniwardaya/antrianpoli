...
class PasienPoliJantungController extends Controller
{
    public function dataAntrianJantung(){
        $today = Carbon::now()->format('Y-m-d');
        
        $dataAntrianJantung = DB::table('reg_periksa')
            ->join('poliklinik', 'reg_periksa.kd_poli', '=', 'poliklinik.kd_poli') 
            ->join('pegawai', 'reg_periksa.kd_dokter', '=', 'pegawai.nik') 
            ->join('pasien', 'reg_periksa.no_rkm_medis', '=', 'pasien.no_rkm_medis')  
            ->where('reg_periksa.kd_poli', 'JAN') 
            ->where('tgl_registrasi', $today)
            ->get();

        return DataTables::of($dataAntrianJantung)
            ->make(true);
    }
    
    public function panggilAntrianJantung(Request $request)
    {
        $today = Carbon::now()->format('Y-m-d');
    ...
    public function selesaiAntrianJantung(Request $request){
        $no_reg = $request->input('no_reg');
        $kd_poli = $request->input('kd_poli');
        $nama_dokter = $request->input('nama');
        $status = 'selesai';
    ...
    public function dashboardJantung(){

        return view('dashboard.dashboardJantung',[

            'nama_poli' => 'POLIKLINIK JANTUNG',
        ]);
    }
    public function dashboardJantung2(){

        return view('dashboard.dashboardJantung2',[

            'nama_poli' => 'POLIKLINIK JANTUNG',
        ]);
    }

    public function getAntrianJantung()
    {
        ...
        $nama_dokter = DB::table('dashboard_poli')
            ...
            ->where('tgl_registrasi', $today)
            ->where('kd_poli', 'JAN')
            ->where('nama_dokter','dr. Irlandi M Suseno, Sp.JP')           
            ->value('nama_dokter');
