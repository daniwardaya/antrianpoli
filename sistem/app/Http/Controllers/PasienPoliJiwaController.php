...
class PasienPoliJiwaController extends Controller
{
    public function dataAntrianJiwa(){
        $today = Carbon::now()->format('Y-m-d');
        
        $dataAntrianJiwa = DB::table('reg_periksa')
            ->join('poliklinik', 'reg_periksa.kd_poli', '=', 'poliklinik.kd_poli') 
            ->join('pegawai', 'reg_periksa.kd_dokter', '=', 'pegawai.nik') 
            ->join('pasien', 'reg_periksa.no_rkm_medis', '=', 'pasien.no_rkm_medis')  
            ->where('reg_periksa.kd_poli', 'JIW') 
            ->where('tgl_registrasi', $today)
            ->get();

        return DataTables::of($dataAntrianJiwa)
            ->make(true);
    }
    
    public function panggilAntrianJiwa(Request $request)
    {
        $today = Carbon::now()->format('Y-m-d');
    ...
    public function selesaiAntrianJiwa(Request $request){
        $no_reg = $request->input('no_reg');
        $kd_poli = $request->input('kd_poli');
        $nama_dokter = $request->input('nama');
        $status = 'selesai';
    ...
    public function dashboardJiwa(){

        return view('dashboard.dashboardJiwa',[

            'nama_poli' => 'POLIKLINIK JIWA',
        ]);
    }
    public function dashboardJiwa2(){

        return view('dashboard.dashboardJiwa2',[

            'nama_poli' => 'POLIKLINIK JIWA',
        ]);
    }

    public function getAntrianJiwa()
    {
        ...
        $nama_dokter = DB::table('dashboard_poli')
            ...
            ->where('tgl_registrasi', $today)
            ->where('kd_poli', 'JIW')
            ->where('nama_dokter','dr. H.Suponco Eddi W., Sp.Kj')           
            ->value('nama_dokter');
    ...
    public function getAntrianJiwa2()
    {
        ...
        $nama_dokter = DB::table('dashboard_poli')
            ...
            ->where('tgl_registrasi', $today)
            ->where('kd_poli', 'JIW')
            ->where('nama_dokter','dr. Eka Puji Lestari. Sp.KJ')
            ->value('nama_dokter');
