...
class PasienPoliKulitController extends Controller
{
    public function dataAntrianKulit(){
        $today = Carbon::now()->format('Y-m-d');
        
        $dataAntrianKulit = DB::table('reg_periksa')
            ->join('poliklinik', 'reg_periksa.kd_poli', '=', 'poliklinik.kd_poli') 
            ->join('pegawai', 'reg_periksa.kd_dokter', '=', 'pegawai.nik') 
            ->join('pasien', 'reg_periksa.no_rkm_medis', '=', 'pasien.no_rkm_medis')  
            ->where('reg_periksa.kd_poli', 'KLT') 
            ->where('tgl_registrasi', $today)
            ->get();

        return DataTables::of($dataAntrianKulit)
            ->make(true);
    }
    
    public function panggilAntrianKulit(Request $request)
    {
        $today = Carbon::now()->format('Y-m-d');
    ...
    public function selesaiAntrianKulit(Request $request){
        $no_reg = $request->input('no_reg');
        $kd_poli = $request->input('kd_poli');
        $nama_dokter = $request->input('nama');
        $status = 'selesai';
    ...
    public function dashboardKulit(){

        return view('dashboard.dashboardKulit',[

            'nama_poli' => 'POLIKLINIK KULIT',
        ]);
    }
