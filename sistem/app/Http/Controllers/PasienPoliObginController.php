...
class PasienPoliObginController extends Controller
{
    public function dataAntrianObgin(){
        $today = Carbon::now()->format('Y-m-d');
        
        $dataAntrianObgin = DB::table('reg_periksa')
            ->join('poliklinik', 'reg_periksa.kd_poli', '=', 'poliklinik.kd_poli') 
            ->join('pegawai', 'reg_periksa.kd_dokter', '=', 'pegawai.nik') 
            ->join('pasien', 'reg_periksa.no_rkm_medis', '=', 'pasien.no_rkm_medis')  
            ->where('reg_periksa.kd_poli', 'OBG') 
            ->where('tgl_registrasi', $today)
            ->get();

        return DataTables::of($dataAntrianObgin)
            ->make(true);
    }
    
    public function panggilAntrianObgin(Request $request)
    {
        $today = Carbon::now()->format('Y-m-d');
    ...
    public function selesaiAntrianObgin(Request $request){
        $no_reg = $request->input('no_reg');
        $kd_poli = $request->input('kd_poli');
        $nama_dokter = $request->input('nama');
        $status = 'selesai';
    ...
    public function dashboardObgin(){
        
      

        

        return view('dashboard.dashboardObgin',[

            'nama_poli' => 'POLIKLINIK KANDUNGAN',
        ]);
    }
    public function dashboardObgin2(){
        
      

        

        return view('dashboard.dashboardObgin2',[

            'nama_poli' => 'POLIKLINIK KANDUNGAN',
        ]);
    }

    public function getAntrianObgin()
    {
        ...
        $nama_dokter = DB::table('dashboard_poli')
            ...
            ->where('tgl_registrasi', $today)
            ->where('kd_poli', 'OBG')
            ->where('nama_dokter','dr. Johan Taruna, Sp.OG')
            ->value('nama_dokter');
    ...
    public function getAntrianObgin2()
    {
        ...
        $nama_dokter = DB::table('dashboard_poli')
            ...
            ->where('tgl_registrasi', $today)
            ->where('kd_poli', 'OBG')
            ->where('nama_dokter','dr. Teuku Kyan Nuryasin, Sp.OG')
            ->value('nama_dokter');
